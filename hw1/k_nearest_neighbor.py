...
class KNearestNeighbor(object):
    ...
    def compute_L2_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                squared_difference=np.sum(np.square(X[i]-self.X_train[j]))
                dists[i,j]=np.sqrt(squared_difference)
    ...
    def compute_L2_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        test_norm=np.sum(X**2,axis=1,keepdims=True)
        train_norm=np.sum(self.X_train**2,axis=1)
        dists=np.sqrt(test_norm-2*np.dot(X,self.X_train.T)+train_norm)
    ...
    def compute_Cosine_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                #####################################################################
                # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
                test_norm=np.sqrt(np.sum(np.square(X[i])))
                train_norm=np.sqrt(np.sum(np.square(self.X_train[j])))
                cos_dist= 1- (np.dot(X[i],self.X_train[j])/(test_norm*train_norm))
                dists[i,j]=cos_dist
    ...
    def compute_Cosine_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            test_norm=np.sqrt(np.sum(np.square(X[i])))
            train_norm=np.sqrt(np.sum(np.square(self.X_train),axis=1))
            cos_dist= 1- (np.dot(X[i],self.X_train.T)/(test_norm*train_norm))
            dists[i]=cos_dist
    ...
    def compute_Cosine_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        test_norm=np.sqrt(np.sum(X**2,axis=1,keepdims=True))
        train_norm=np.sqrt(np.sum(self.X_train**2,axis=1))
        cos_dist=1- np.dot(X,self.X_train.T)/(test_norm.dot(train_norm.reshape(1,-1)))
        dists=cos_dist
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            knn_index=index[:k]
            label=self.y_train[knn_index]
            closest_y=list(label)
            ...
            count=np.array([np.sum(closest_y==label) for label in labels])
            most_common=labels[np.argmax(count)]
